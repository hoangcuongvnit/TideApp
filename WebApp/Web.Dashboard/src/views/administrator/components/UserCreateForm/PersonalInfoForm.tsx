...
const PersonalInfoForm = (props: PersonalInfoFormProps) => {
    ...
    const { roles, webApplications, values, touched, errors, message, resetForm } = props
    const validatorProps = { touched, errors }
    const roleOptions: Option[] = map(roles, (role) => ({ value: role.id, label: t(`UI_${role.localeKey}`, role.name) }))
    const webAppOptions: Option[] = [{ value: '', label: 'Default' }, ...map(webApplications, (webApp) => {
        return { value: webApp.id, label: webApp.name } as Option
    })]
    const adminRoleId = roles.find((role) => role.name === Roles.Administrator)?.id

    useEffect(() => {
        if (values.roleId === adminRoleId) {
            values.webApplication = ''
        }
                ...
                <Field
                    ...
                    {({
                        ...
                        <Select
                            ...
                            isDisabled={values.roleId === adminRoleId}
                        />
                    )}
                </Field>
            </FormRow>
